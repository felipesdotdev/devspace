...
export function loadDevspaceFiles(env: NodeJS.ProcessEnv = process.env): DevspaceFiles {
  ...
  return {
    ...
    configExists,
    authExists,
    config: configExists ? readJson<DevspaceUserConfig>(configPath) : {},
    auth: authExists ? readJson<DevspaceAuthConfig>(authPath) : {},
  };
}

export function saveDevspaceConfig(config: DevspaceUserConfig, env: NodeJS.ProcessEnv = process.env): string {
...
export function saveDevspaceAuth(auth: DevspaceAuthConfig, env: NodeJS.ProcessEnv = process.env): string {
  const filePath = devspaceAuthPath(env);
  mkdirSync(devspaceConfigDir(env), { recursive: true });
  writeFileSync(filePath, JSON.stringify(auth, null, 2) + "\n", { mode: 0o600 });
  return filePath;
}
